...
class RegisterAs extends React.Component {
    ...
    render() {
        return (
            ...
            <Link to="/user-sign-up"><ButtonPage name="Sign Up As a User"
             onclick={()=>this.props.login(this.state,this.props.history)}/></Link>
            <Link to="/restro-sign-up"><ButtonPage name="Sign Up As Resturent" 
            onclick={()=>this.props.login(this.state,this.props.history)}/></Link>
            </div>
        </div>
                    </div>
